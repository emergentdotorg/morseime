...
public class KeyboardSwitcher {
  ...
  private int mTextMode = MODE_TEXT_ALPHA; // MODE_TEXT_QWERTY;
  ...
  void setInputView(MorseKeyboardView inputView) {
  ...
  void setKeyboardMode(int mode, int imeOptions) {
    ...
    setKeyboardMode(mode == MODE_SYMBOLS ? MODE_TEXT : mode, imeOptions, mPreferSymbols);
  }

  void setKeyboardMode(int mode, int imeOptions, boolean isSymbols) {
    setKeyboardMode(mode, imeOptions, isSymbols, mTextMode == MODE_TEXT_QWERTY);
  }

  void setKeyboardMode(int mode, int imeOptions, boolean isSymbols, boolean isQwerty) {
    mMode = mode;
    mImeOptions = imeOptions;
    mIsSymbols = isSymbols;
    mInputView.setPreviewEnabled(true);
    mTextMode = isQwerty ? MODE_TEXT_QWERTY : MODE_TEXT_ALPHA;
  ...
  private KeyboardId getKeyboardId(int mode, int imeOptions, boolean isSymbols) {
    ...
    int testKbdRes = mTextMode == MODE_TEXT_QWERTY ? R.xml.kbd_qwerty : R.xml.kbd_morse;

    switch (mode) {
      case MODE_TEXT:
        return new KeyboardId(testKbdRes, KEYBOARDMODE_NORMAL, true);
      case MODE_SYMBOLS:
        return new KeyboardId(R.xml.kbd_symbols);
      case MODE_PHONE:
        return new KeyboardId(R.xml.kbd_phone);
      case MODE_URL:
        return new KeyboardId(testKbdRes, KEYBOARDMODE_URL, true);
      case MODE_EMAIL:
        return new KeyboardId(testKbdRes, KEYBOARDMODE_EMAIL, true);
      case MODE_IM:
        return new KeyboardId(testKbdRes, KEYBOARDMODE_IM, true);
    }
    return null;
  }
  ...
  void toggleSymbols() {
    if (mIsSymbols) {
      setKeyboardMode(mMode, mImeOptions, false, false);
    } else {
      if (mTextMode != MODE_TEXT_QWERTY) {
        setKeyboardMode(mMode, mImeOptions, false, true);
      } else {
        setKeyboardMode(mMode, mImeOptions, true);
      }
    }
